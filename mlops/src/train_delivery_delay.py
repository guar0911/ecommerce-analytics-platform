"""
Train a delivery delay classifier from the OLAP feature table
(analytics.ml_delivery_delay_features) and log everything to MLflow.

Usage:
    python mlops/src/train_delivery_delay.py                       # uses .env (local Postgres)
    python mlops/src/train_delivery_delay.py --env-file .env.supabase   # trains from Supabase
"""
...
matplotlib.use("Agg")  # no display needed, we only save the plot as a file
...
EXPERIMENT_NAME = "delivery_delay_classification"
...
def load_data(env_file: str) -> pd.DataFrame:
    ...
    df = pd.read_sql("SELECT * FROM analytics.ml_delivery_delay_features", engine)
    print(f"Loaded {len(df):,} rows")
    return df


def prepare_features(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    # Drop rows missing critical geo/category info rather than guessing values
    # for a first version of the model.
    before = len(df)
...
def main() -> None:
    ...
    params = {
        ...
        "max_depth": 12,
        "min_samples_leaf": 5,
        "class_weight": "balanced",  # compensates for the 92/8 class imbalance
        "random_state": 42,
    }
